...
def login(page):
    try:
        ...
        logger.info("Manually navigated to Corporate Tax page")
...
def add_hst_for_client(page, client_name, ct_method="Monthly"):
    try:
        ...
        page.click(f'text="{ct_method}"')
        logger.info(f"CT Method selected: {ct_method}")
        ...
        logger.info(f"CT added for client {client_name} with method {ct_method}")

    except Exception as e:
        error_logger.error(f"Error occurred while adding CT: {str(e)}", exc_info=True)
...
if __name__ == "__main__":
    logger.info("********** CT addition Test Case **********")
    main()
