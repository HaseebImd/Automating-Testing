...
def add_payroll_for_client(page, client_name, payroll_method="Bimonthly"):
    logger.info("********** Payroll addition Test Case Running **********")
    try:
        # Ensure we are on the HST page
        page.goto(f"{base_url}/apps/payrolls/")
        time.sleep(5)
        ...
        page.click(f'text="{payroll_method}"')
        ...
        logger.info(f"HST added for client {client_name} with method {payroll_method}")
        logger.info("********** Payroll addition Test Case Passed **********")

    except Exception as e:
        logger.info("********** Payroll addition Test Case Failed **********")
        error_logger.error(f"Error occurred while adding Payroll: {str(e)}", exc_info=True)
